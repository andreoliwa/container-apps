...
- `${CONTAINER_APPS_DATA_DIR}/immich/library` — bind-mounted upload dir (back this up)
- `IMMICH_DB_PASSWORD` env var
- `CONTAINER_APPS_DATA_DIR` env var
...
| -------------------- | ------------------------------------------- | ------------------------------------------------------ |
| Uploaded originals   | `${CONTAINER_APPS_DATA_DIR}/immich/library` | Yes — back up this dir                                 |
...
1. `rsync -v immich/ rlyeh:~/container-apps/immich/`
2. Also rsync updated `tasks.py` and `redis/compose.yaml`
3. Run `invoke immich-setup` on Hetzner (directory creation)
4. Transfer library: `rsync -av ${CONTAINER_APPS_DATA_DIR}/immich/ rlyeh:~/data/immich/`
