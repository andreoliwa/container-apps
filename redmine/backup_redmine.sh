...
/usr/local/bin/docker-compose -f $HOME_DIR/container-apps/postgres/docker-compose.yml exec -T postgres12 pg_dump -U postgres redmine > "$BACKUP_DIR/redmine_$(date "+%Y-%m-%d-%H-%M-%S").sql"
ls -l "$BACKUP_DIR"
